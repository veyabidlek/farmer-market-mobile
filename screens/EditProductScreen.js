...
import { editFarmerProduct } from "../api/farmer/editFarmerProduct";
...
const EditProductScreen = ({ route, navigation }) => {
  ...
  const [category, setCategory] = useState(
    product.category_id?.toString() || "1"
  );
  const [price, setPrice] = useState(product.price?.toString() || "");
  const [quantity, setQuantity] = useState(product.quantity?.toString() || "");
  const [description, setDescription] = useState(product.description || "");
  const [iosImage, setIosImage] = useState(
    product.image_url ? { uri: product.image_url } : null
  );
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      if (iosImage && iosImage.uri !== product.image_url) {
        try {
      ...
      const success = await editFarmerProduct(product.id, productData);
